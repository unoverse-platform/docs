...
> **Status**: 🟢 Live end-to-end (July 2026), schema + guards + Studio's publish gate.
...
| **Studio lint** | **at publish** (Studio blocks on any error) | everything below, with doc-cited messages; 0 errors required | Studio's publish gate |
...
## 3. Studio's lint + the guard tests (one rule set, two homes)

Studio's lint runs at publish time and **mirrors** the server guards
...
- **Lint and guards must agree.** Studio's lint mirrors `microapp-structure`/`self-guard`/
...
- **Never trade a false positive for a catch.** The suite is only trusted while every valid
  file passes: re-run a Studio publish check + `npm test` after any change.
