...
**Build cross-platform, server-driven UI (SDUI) as pure data: components and apps that render natively on every channel, streamed by real agents over MCP.**
...
| [02: SDUI & MCP Apps](/design/sdui-and-mcp-apps) | The model: UI is data, the closed primitive set, why apps ARE MCP Apps |
...
| [05: Apps (MCP Apps)](/design/apps) | App shells: the manifest state tree (the priority ladder), `ComponentSlot`, `Timeline`, `whenToUse` |
...
| Define a whole surface (chat, voice, dashboard) | **App** (`design/<project>/apps/`) | [05](/design/apps) |
...
| **The reaction contract** | A component is a small state machine that writes only its own slice (`setValue`); apps react by name, in their declared priority order; **inline is the universal default**. |
...
| **MCP is the standard** | Apps are MCP Apps: definitions are MCP resources, sends are `tools/call`, answers are elicitations. No bespoke transport. |
