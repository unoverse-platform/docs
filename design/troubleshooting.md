...
| Style silently ignored | raw value (`12px`, `#fff`) or a token name that doesn't exist in the org's semantic set | tokens only; check `design/<project>/styles/semantic/` for the real name; Studio's publish lint catches raw values |
