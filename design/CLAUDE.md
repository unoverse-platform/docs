...
Exemplars (as shipped): `design/sab/components/product-card` (component tree: `products {}` + `detail` with private steps) · `design/sab/templates/sab-chat-layout/manifest.yaml` (template tree: `main>welcome` contained; ladder `focus/products/detail`).
