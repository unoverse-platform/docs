...
**THE CONTAINER MODEL (2026-08-25, doc of record: [UNOVERSE_INTERFACE_MODEL](/unoverse/UNOVERSE_INTERFACE_MODEL)) — the target shape for NEW apps; the app-tree form above remains as-built for existing ones.** An app = an arrangement of CONTAINERS; it has NO states, NO ladder, NO `preview:`, NO app-level `binding`/`inputSchema`, and its one layout is `layouts/main.yaml` (no `layout:` key). The manifest is identity + discovery + `containers:`. A container's STATES each HOLD AN INTERFACE (`page: { interface: deal }`; `chat: { interface: text-chat, workflow: …, inputSchema: … }` — the schema lives WITH the workflow that consumes it); a state may hold NOTHING (`closed: {}` = a collapsed drawer — collapse is the CONTAINER's arrangement, NEVER a state on the interface). The face shown = the container state's own name when the interface declares it, else explicit `state:`. Active container state = app chrome key `container:<name>` via `setAppValue` (Studio draws per-container tabs; runtime handles are authored buttons). Mocks = the interface's OWN defaults seeded into its face — never authored preview data. The layout places containers by name (`type: Container, name: deals`); width belongs to the interface's active state — layout boxes hug (`flex: 0 0 auto`) or absorb (`flex: 1`), and a container app has no app-level resting width. Exemplar: `design/better-alpha/apps/deal-workspace`.
