...
Condensed, non-negotiable rules for building Unoverse components and templates. Full journey: [README](/design/overview), docs 01–09. Deep reference: `docs/unoverse/UNOVERSE_AUTHORING.md`, `UNOVERSE_STATE_MODEL.md` (§5b = the reaction contract), `UNOVERSE_LAYERS.md`, `UNOVERSE_CONFORMANCE.md`.
...
named layout, `layouts: [...]`-scoped). Declared in the manifest's `lifecycle` array;
...
changing its own view writes `defaultState`, and that write IS the event the hook reacts
...
6. **A component owns its faces and size; the template owns only the framing.** No component-type rules in templates.
...
1. Read the matching journey doc ([03](/design/components) component / [05](/design/templates) template / [06](/design/styles-and-tokens) styles); study the exemplars: `productfinder`/`planfinder` (components), `acmechatlayout` (template).
